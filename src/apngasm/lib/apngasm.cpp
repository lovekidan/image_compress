...
// #define PNG_COLOR_TYPE_PALETTE  (PNG_COLOR_MASK_COLOR | PNG_COLOR_MASK_PALETTE)       3
// #define PNG_COLOR_TYPE_RGB        (PNG_COLOR_MASK_COLOR)                              2
// #define PNG_COLOR_TYPE_RGB_ALPHA  (PNG_COLOR_MASK_COLOR | PNG_COLOR_MASK_ALPHA)       6
// #define PNG_COLOR_TYPE_GRAY_ALPHA (PNG_COLOR_MASK_ALPH                                4
	...
	int APNGAsm::upconvertToCommonType(unsigned char coltype)//向上扩展为统一颜色类型
	{
		unsigned char  * sp;
		unsigned char  * dp;
		unsigned char    r, g, b;
		...
		for (size_t n = 0; n < _frames.size(); ++n)
		{
			if (coltype == 6 && _frames[n]._colorType != 6)
			{
				...
				if (_frames[n]._colorType == 0)//灰度图转RGBA
				{
					...
					if (_frames[n]._colorType == 2)//RGB转RGBA
					{
						sp = _frames[n]._pixels;
						dp = dst;
						if (_frames[n]._transparencySize == 0)
						...
						if (_frames[n]._colorType == 3)//调色板转RGBA
						{
							sp = _frames[n]._pixels;
							dp = dst;
				...
				if (coltype == 4 && _frames[n]._colorType == 0)//灰度转GA
				{
					...
					if (coltype == 2 && _frames[n]._colorType == 0)//灰度转RGB
					{
	...
	void APNGAsm::dirtyTransparencyOptimization(unsigned char coltype)//清除透明像素的颜色
	{
		if (coltype == 6)
		{
			for (size_t n = 0; n < _frames.size(); ++n)
	...
	unsigned char APNGAsm::downconvertOptimizations(unsigned char coltype, bool keep_palette, bool keep_coltype)
	{
		...
		if (coltype == 6 && !keep_coltype)
		{
            ...
            //生成公共调色板
			for (size_t n = 0; n < _frames.size(); ++n)//对于每一帧
			{
				sp = _frames[n]._pixels;
				for (j = 0; j < _size; j++)//每个像素
				{
					r = *sp++;
					g = *sp++;
					b = *sp++;
            ...
            if (grayscale && simple_trans && colors <= 256) /* 6 -> 0 *//*RGBA转灰度*/
			{
				coltype = 0;
				...
				for (size_t n = 0; n < _frames.size(); ++n)//每一帧
				{
					sp = dp = _frames[n]._pixels;
					for (j = 0; j < _size; j++, sp += 4)//每个像素
					{
						if (sp[3] == 0)
							*dp++ = _trns[1];
						else
	...
	{
		unsigned int bpp = 1;//byte per pixel
		if (coltype == 2)
			bpp = 3;
		else
			if (coltype == 4)
	...
	void APNGAsm::deflate_rect_fin(unsigned char * zbuf, unsigned int * zsize, int bpp, int stride, unsigned char * rows, int zbuf_size, int n)
	{
		...
		if (deflate_method == 2)
		{
			...
			ZopfliCompress(&opt_zopfli, ZOPFLI_FORMAT_ZLIB, rows, _op[n].h*(rowbytes + 1), &data, &size);
		...
		else
		{
			...
			deflateInit2(&fin_zstream, Z_BEST_COMPRESSION, 8, 15, 8, _op[n].filters ? Z_FILTERED : Z_DEFAULT_STRATEGY);

			fin_zstream.next_out = zbuf;
			fin_zstream.avail_out = zbuf_size;
			fin_zstream.next_in = rows;
			fin_zstream.avail_in = _op[n].h*(rowbytes + 1);
