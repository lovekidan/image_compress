...
int CJpegFile(const char *fileName, const char *outName, int quality)
{
    if (!fileName || !strlen(fileName) || !outName || !strlen(outName))
    {
        return -1;
    }
    ...
    char *output = malloc(strlen(outName) + 1);
    strcpy(output, outName);


    char strQuality[32] = { 0 };
    snprintf(strQuality, 32, "%.2d", quality);

    const char* argvCjpeg[] = {
        "cjpeg",
        "-quality",
        strQuality,
        "-outfile",
