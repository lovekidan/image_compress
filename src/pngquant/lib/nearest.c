...
static vp_node *vp_create_node(mempool *m, vp_sort_tmp *indexes, int num_indexes, const colormap_item items[]) {
    ...
    if (num_indexes == 1) {
        /*
		** lovekid: complier may not support this commnet.
		*node = (vp_node){
            .vantage_point = items[indexes[0].idx].acolor,
            .idx = indexes[0].idx,
            .radius = MAX_DIFF,
        };*/
		node->vantage_point = items[indexes[0].idx].acolor;
		node->idx = indexes[0].idx;
		node->radius = MAX_DIFF;
		node->near = 0;
		node->far = 0;
	...
	*/
	node->vantage_point = items[ref_idx].acolor;
	node->idx = ref_idx;
	node->radius = sqrtf(indexes[half_idx].distance_squared);
